...
{
    public partial class DeliveryLowPriceList : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string IsPostAcc = "";
            if (cbxDelvStatus.SelectedValue == "1") { IsPostAcc = "N"; };
            if (cbxDelvStatus.SelectedValue == "2") { IsPostAcc = "Y"; };

            DADelivery cls = new DADelivery();
            DataTable dt = cls.GetDataDeliveryItem(txtDeliveryNo.Text, ddlProject.SelectedItem.Value, ddlItem.SelectedItem.Value, dateBeg, dateEnd, "", "Y", "Y", IsPostAcc);
            Session["dtDelvPost"] = dt;
            DataTable dtGrd = dt.DefaultView.ToTable(true, "DelvLstId", "isConfirm", "isPostAcc", "DelvPromotionId", "DelvDate", "ItemId", "ItemName", "CostCenter", "PostRet_Key", "PostRetKey", "ProjectCode", "ProjectID", "ProjectName", "WBS", "CompanySAPCode");
            dtGrd.Columns.Add(new DataColumn("Amount", typeof(int)));
            foreach (DataRow dr in dtGrd.Rows)
            {
                string cond = "ISNULL(DelvLstId,'') = '" + dr["DelvLstId"] 
                    + "' AND ISNULL(isConfirm,'') = '" + dr["isConfirm"] 
                    + "' AND ISNULL(isPostAcc,'') = '" + dr["isPostAcc"] 
                    + "' AND ISNULL(DelvPromotionId,'') = '" + dr["DelvPromotionId"] 
                    + "' AND ISNULL(DelvDate,'') = '" + dr["DelvDate"] 
                    + "' AND ISNULL(ItemId,'') = '" + dr["ItemId"]
                    + "' AND ISNULL(ItemName,'') = '" + dr["ItemName"]
                    + "' AND ISNULL(PostRetKey,'') = '" + dr["PostRetKey"] 
                    + "' AND ISNULL(ProjectCode,'') = '" + dr["ProjectCode"] 
                    + "' AND ISNULL(WBS,'') = '" + dr["WBS"] 
                    + "' AND ISNULL(CompanySAPCode,'') = '" + dr["CompanySAPCode"] 
                    + "' AND ISNULL(ProjectID,'') = '" + dr["ProjectID"] + "'";
                dr["Amount"] = dt.Compute("COUNT(ItemName)", cond);
            }
            dtGrd.AcceptChanges();
        ...
        protected void btnPostAccountLowPrice_Click(object sender, EventArgs e)
        {
            keepSelDelv();
            string selDelv = Session["selDelv"] + "";
            if (selDelv == "")
            ...
            //DataView dv = dtGrd_C.DefaultView;
            //dv.RowFilter = "DelvLstId in (" + selDelv + ")";
            ...
            if (dtGrd_C.DefaultView.ToTable(true, "CompanySAPCode").Rows.Count != 1)
            {
                ...
                ScriptManager.RegisterStartupScript(this, GetType(), "js", "alert('กรุณาเลือกส่งมอบจากบริษัทเดียวเท่านั้น !!');", true);
                return;
            }
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "js", "OpenColorBox('DeliveryLowPricePostSum.aspx?DelvLstId=" + selDelv + "','91%','79%');", true);
                return;
            }
            else
            {
                // ScriptManager.RegisterStartupScript(this, GetType(), "js", "OpenColorBox('DeliveryLowPricePostSum.aspx?DelvLstId=" + selDelv + "','91%','75%');", true);
                ScriptManager.RegisterStartupScript(this, GetType(), "js", "OpenColorBox('DeliveryLowPricePostSumCrossCmp.aspx?DelvLstId=" + selDelv + "','91%','79%');", true);
                return;
            }
        }
        ...
        protected void grdData_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName + "" == "PrtDoc")
            {
                ...
                {
                    urlRptPstAccIO = "frmReport.aspx?reqRPT=PstAccIOCrs";
                }
        ...
        [WebMethod]
        public static object[] ReversingAccountsIO(string docid, string remark, string postingDate, string reasonId, string reasonName, string empid)
        {
            try
            {
                Class.DADelivery dadrv = new Class.DADelivery();
                object[] objData = dadrv.GetDataForReverseDocIO(docid);
                ...
                if (Mode == "0")
                {
                    ...
                    if (msgErr == "")
                    {
                        string msgerr = dadrv.InsertReverseAccountIO(docid, remark, postingDate, reasonId, reasonName, revSapDocNo, empid);
                        if (msgerr == "")
                        {
                            return new object[] {
                ...
                else
                {
                    ...
                    if (msgErr == "")
                    {
                        string msgerr = dadrv.InsertReverseAccountIOCrossCenter(docid, remark, postingDate, reasonId, reasonName,
        ...
        public static object[] GetAccountingRecorded()
        {
            ...
            {
                Class.DADelivery dadrv = new Class.DADelivery();
                DataTable dt = (DataTable)dadrv.GetAccountingRecordedIO()[0];
